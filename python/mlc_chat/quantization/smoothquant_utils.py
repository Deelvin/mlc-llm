...
from slm.compiler_pass.smoothquant import SmoothQuantAnnotator, SmoothQuantStatCollector
...
def get_runtime_func(funcs: List[str], mod: tvm.IRModule):
    ...
    print("------get_runtime_func-------")
    print("mod is ", mod)
    seq = relax.get_pipeline("mlc_calibrate", target=target )
    # mod = relax.transform.LegalizeOps()(mod)
    # lowered_mod = tvm.relax.transform.DeadCodeElimination(funcs)(mod)
    # if target_kind != "cpu":
    #     with target:
...
def _smooth(
    ...
    prefill, decode, kvc = get_runtime_func(module_names, stat_mod)
...
def smoothquant(
    ...
    calibrate_target = tvm.target.Target("nvidia/nvidia-a10g", host="llvm")
    smq_device = tvm.device(calibrate_target.kind.name, 0)

    # smq_device = tvm.device(target.kind.default_keys[0])
    # params = to_device(cpu_params, smq_device)
    # # Free memory on the host:
    # cpu_params.clear()
    artifact_path = '/home/sshtin/dev/mlc-serve/dist/Llama-2-7b-chat-hf-smq_q8i8f16_0-vllm-1gpu/'
    dataset, stop_tokens = _get_dataset("dummy", artifact_path, device=smq_device)
    ...
    smq_config["sliding_window"] = None#args.sliding_window
    with calibrate_target:
        print("[SmoothQuant] Run smoothing...")
        calibrate_mod, s_scales = _smooth(mod, params, dataset, module_names, smq_config)
